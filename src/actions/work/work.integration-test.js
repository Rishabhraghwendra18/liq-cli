...
test('no action results in error and work help', () => {
  ...
  const result = shell.exec(`liq work`, execOpts)
...
test("'help work' prints work help", () => {
  const result = shell.exec(`liq help work`, execOpts)
...
test("'work start' should require additional arguments", () => {
  const result = shell.exec(`cd ${testCheckoutDir} && liq work start`, execOpts)
  const expectedErr = expect.stringMatching(
...
test("'work start add-feature' result in new branch", () => {
  const result = shell.exec(`cd ${testCheckoutDir} && liq work start add-feature`)
...
test("'work merge' results merge, push, and deleting branch", () => {
  ...
  const result = shell.exec(`cd ${testCheckoutDir} && liq work merge`)
