...
_catalyst()
{
    ...
    case "${prev}" in
      ...
      project)
        opts="requires-service service import setup setup-scripts build start lint lint-fix test npm-check npm-update qa link link-dev close deploy add-mirror set-billing ignore-rest";;
      sql)
