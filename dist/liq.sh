...
projects-sync() {
  ...
  if [[ "$CURR_BRANCH" != "master" ]]; then
    ...
    if [[ -z "$NO_WORK_MASTER_MERGE" ]] \
      ...
      if git diff-index --quite HEAD "${BASE_DIR}" && git diff --quiet HEAD "${BASE_DIR}"; then
        ...
        if ! git diff-index --quiet HEAD "${BASE_DIR}/dist" || ! git diff --quite HEAD "${BASE_DIR}/dist"; then # there are changes in ./dist
